...
def detr_resnet50(pretrained=False, num_classes=91, return_postprocessor=False):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/detr/detr-r50-e632da11.pth", model_dir='model/checkpoints/', map_location="cpu", check_hash=True
        )
...
def detr_resnet50_dc5(pretrained=False, num_classes=91, return_postprocessor=False):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/detr/detr-r50-dc5-f0fb7ef5.pth", model_dir='model/checkpoints/', map_location="cpu", check_hash=True
        )
        model.load_state_dict(checkpoint["model"])
    if return_postprocessor:
        return model, PostProcess()
...
def detr_resnet101(pretrained=False, num_classes=91, return_postprocessor=False):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/detr/detr-r101-2c7b67e5.pth", model_dir='model/checkpoints/', map_location="cpu", check_hash=True
        )
        model.load_state_dict(checkpoint["model"])
    if return_postprocessor:
        return model, PostProcess()
...
def detr_resnet101_dc5(pretrained=False, num_classes=91, return_postprocessor=False):
    ...
    if pretrained:
        checkpoint = torch.hub.load_state_dict_from_url(
            url="https://dl.fbaipublicfiles.com/detr/detr-r101-dc5-a2e86def.pth", model_dir='model/checkpoints/', map_location="cpu", check_hash=True
        )
        model.load_state_dict(checkpoint["model"])
    if return_postprocessor:
        return model, PostProcess()
